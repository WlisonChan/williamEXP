...
public class Kmeans {
    ...
    private static List<Cluster> initCentroides(List<Point> points, Integer k) {
        ...
        double max_X = Float.NEGATIVE_INFINITY;
        double max_Y = Float.NEGATIVE_INFINITY;
        double min_X = Float.POSITIVE_INFINITY;
        double min_Y = Float.POSITIVE_INFINITY;
        for (Point point : points) {
            max_X = max_X < point.getValue() ? point.getValue() : max_X;
            max_Y = max_Y < point.getQuality() ? point.getQuality() : max_Y;
            min_X = min_X > point.getValue() ? point.getValue() : min_X;
            min_Y = min_Y > point.getQuality() ? point.getQuality() : min_Y;
        }
        ...
        // 随机初始化k个中心点
        for (int i = 0; i < k; i++) {
            double x = random.nextFloat() * (max_X - min_X) + min_X;
            double y = random.nextFloat() * (max_Y - min_Y) + min_X;
            Cluster c = new Cluster();
            Point centroide = new Point((float)0, (float)0); // 初始化的随机中心点
    ...
    private static void recalcularCentroides(List<Cluster> clusters) {
        for (Cluster c : clusters) {
            ...
            for (Point point : c.getPoints()) {
                sum_x += point.getValue();
                sum_y += point.getQuality();
            }
            x = sum_x / c.getPoints().size();
            y = sum_y / c.getPoints().size();
            Point nuevoCentroide = new Point((float) 0, (float)0); // 新的质心点
            nuevoCentroide.setValue(x);
            nuevoCentroide.setQuality(y);

            if (x == c.getCentroid().getValue() && y == c.getCentroid().getQuality()) { // 如果质心点不再改变 则该分类已经收敛
                c.setConvergence(true);
            } else {
